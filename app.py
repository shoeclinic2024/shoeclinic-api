...
def login():
    ...
    if security_service.is_rate_limited(ip_address):
        ...
        flash(f"≡ƒ¢í∩╕Å Too many login attempts. Please try again in {minutes_remaining} minutes.", "danger")
    ...
    if security_service.is_account_locked(user):
        ...
        flash(f"≡ƒöÆ Account locked due to multiple failed attempts. Try again in {minutes_remaining} minutes.", "danger")
    ...
    if not bcrypt.check_password_hash(user.password, password):
        ...
        if is_locked:
            flash(f"≡ƒöÆ Account locked after {security_service.MAX_LOGIN_ATTEMPTS} failed attempts. Locked for {security_service.LOCKOUT_DURATION_MINUTES} minutes.", "danger")
    ...
    if not user.is_active:
        flash("ΓÅ│ Account pending approval. Please contact the Owner.", "warning")
...
def check_session_timeout():
    ...
    if current_user.is_authenticated:
        if security_service.check_session_timeout():
            logout_user()
            session.clear()
            flash("ΓÅ▒∩╕Å Session expired due to inactivity. Please login again.", "info")
            return redirect(url_for("login_page"))
...
def tsc_assist_query():
    ...
    if current_user.role in ['admin', 'super_admin']:
        ...
        if query_type == "stats_pending":
            ...
            return jsonify({"status": "success", "message": f"Today's total billed amount is **Γé╣{revenue:,.2f}**."})
    ...
    if query_type == "search":
        ...
        for o in results:
            msg += f"ΓÇó **{o.job_id}**: {o.customer_name} ({o.status})\n"
...
def export_report_pdf(data, report_type, filename_prefix):
    ...
    if trend:
        ...
        t_header = ['Period', 'Net Income (Γé╣)', 'Operational Exp (Γé╣)', 'Vendor Payout (Γé╣)', 'Net Profit (Γé╣)']
        t_rows = [t_header]
